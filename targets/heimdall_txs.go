...
func UnconfimedTxns(ops types.HTTPOptions, cfg *config.Config, c client.Client) {
	...
	_ = db.WriteToInfluxDb(c, bp, "heimdall_unconfirmed_txns", map[string]string{}, map[string]interface{}{"unconfirmed_txns": totalUnconfirmedTxns})
...
func ValidatorGas(ops types.HTTPOptions, cfg *config.Config, c client.Client) {
	...
	_ = db.WriteToInfluxDb(c, bp, "heimdall_auth_params", map[string]string{}, map[string]interface{}{"max_tx_gas": maxTxGas})
